...
import com.etycx.marry.social.util.*;
...
public class SocialCallBackController extends BaseController{
	...
	@Resource
    private MarryUserService userService;

	private MarryUser marryUser;
	...
	public void qq() throws Exception {
		...
		marryUser = userService.getUserByOpenId(openId, Constans.OpenIdType.QQ);
        if(marryUser == null){
	...
	public void wechat() throws Exception { // web and mobileweb common method
		...
		marryUser = userService.getUserByOpenId(openId, Constans.OpenIdType.WECHAT);
		if(marryUser == null) {
	...
	private void sinaWeiboLoginAction(String code) throws WeiboException, IOException {
		...
		marryUser = userService.getUserByOpenId(uid, Constans.OpenIdType.WEIBO);
		if(marryUser == null) {
			Users um = new Users();
	        ...
	        chkLogin(uid, Constans.OpenIdType.WEIBO, screenName);
		}
	}
	
	private void getQQUserInfoByAccessToken(String accessToken, String openID) throws Exception{
        String url = Global.getConfig("getUserInfoURL") + "?access_token="+accessToken+"&oauth_consumer_key="+Global.getConfig("app_ID")+"&openid="+openID;
        String userinfo = HttpClientUtils.sendGetRequest(url, "utf-8");
        JSONObject jsonUserInfo = JSONObject.fromObject(userinfo);
        chkLogin(openID, Constans.OpenIdType.QQ, jsonUserInfo.getString("nickname"));
	}
	...
	private void wechatResultJson() throws Exception {		
		...
		chkLogin(openId, Constans.OpenIdType.WECHAT, userInfo.getNickname());
	}
	...
	public void chkLogin(String openId, Constans.OpenIdType openIdType, String name) throws IOException {
		MarryUser newUser = new MarryUser();
		if(StringUtils.isNotBlank(name)) {
			name = new String(name.getBytes("UTF-8"), "GBK");
		}
		newUser.setName(name);
		newUser.setOpenId(openId);
		newUser.setOpenType(openIdType);
		userService.insertSelective(newUser);
	}

	public MarryUser getMarryUser() {
		return marryUser;
	}

	public void setMarryUser(MarryUser marryUser) {
		this.marryUser = marryUser;
	}
}
